...
class BottomDescriptionRender {
  ...
  private groupsRenders: Map<string, BottomDescriptionGroupsRender> = new Map()
  ...
  public calcHeight() {
    this.height = Array.from(this.groupsRenders.values()).reduce((sum, groupRenderer) => (sum + groupRenderer.calcHeight()), 0)
    this.container
  ...
  private getChildrenRender(parentId: string, container) {
    let render = this.groupsRenders.get(parentId)
    if (!render) {
      render = new BottomDescriptionGroupsRender(container, {})
      this.groupsRenders.set(parentId, render)
    }
    return render
  }
